...
export const useThemeToggle = ({
  ...
  const setCrazyLightTheme = useCallback(() => {
    ...
    document.startViewTransition(switchTheme);
  }, [setTheme, variant, start, blur, updateStyles]);
...
export const ThemeToggleButton = ({
  ...
  return (
    <button
      ...
      className={cn(
        "size-10 cursor-pointer rounded-full transition-all duration-300 active:scale-95 p-[5px]",
        (theme==="dark") ? "bg-black text-white" : "bg-slate-900/0 text-white",
        className,
      )}
      onClick={toggleTheme}
